...
until the file is committed. This is deliberate: a Featured Sponsor pin
...
4. Reload the page. Everything should load instantly: schedule browsable,
   ...
   still starred. Look at the **Featured Sponsor squares**: each should
...
| `content/logos/` | Sponsor images, named for the sponsor's id and never in the sheet: the wordmark `<id>.svg` (or `.png`, `.jpg`, `.webp`), and for a sponsor that draws a Featured Sponsor pin the square mark `<id>-pin.svg` or `<id>-pin.png` (see "Sponsor logos and pin marks") |
