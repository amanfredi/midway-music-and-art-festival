...
content/
  ...
  logos/<id>-pin.*      # square pin marks, for sponsors that draw a featured pin
...
site/                   # the deploy root; everything in here ships
  ...
  assets/sponsors/*     # logos and pin marks, copied from content/logos/ by build.mjs
...
- **Pin marks are not a column either.** A sponsor that draws a Featured
  Destination pin (see Map contract) supplies a second file beside its logo:
  `content/logos/<id>-pin.<ext>`, `<ext>` one of `svg` or `png` only — those are
  the two formats whose dimensions the build can read from a header without an
  image library, and the build has no dependencies. It is bundled to
  `site/assets/sponsors/<id>-pin.<ext>` and the JSON gets that path as `mark`.
  Copying is the **only** transformation: no resize, no re-encode, so an
  unchanged mark cannot move `content.json`.
  ...
  - Two files differing only in extension is an error, as for logos. At most
    **64 KB** — an eighth of the logo cap, because it ships beside the logo on
    every phone. The same script-rejection rule as logos applies to SVG marks.
  - An SVG mark whose root lacks explicit `width` and `height` attributes is an
    error. A `viewBox` alone is not enough: Safari draws nothing into a canvas
    from such an SVG, and the pin is a canvas image, so the mark would be
    missing on iPhones only.
  - Reported without failing: a raster mark under **128 px** on its longer side
    (256 px is the target; the pin draws it at ~21 CSS px, which is 63 device px
    at 3×), an aspect beyond **2:1** either way (contain-fit leaves a very wide
    mark a sliver), and a `-pin` file present for a sponsor that draws no
    featured pin. These are judgements about how a mark will look on a phone,
    which is not a thing a build can decide and not a thing it should block a
    deploy over.
...
Optional missing fields become `""` (never absent keys, never null) — with two
exceptions, both on sponsors and both because the map branches on absence
rather than merely rendering less: `lat`/`lng` are `null` (never `""` or
absent) when there is no `location`, and numbers when there is; and `mark` is
...
- **Pin types**, diamond-shaped but for the featured square (pins and legend
  icons alike), brand colors:
  ...
  **Venue and featured pins share the largest size: the venue diamond is the
  ...
  sponsor pins. Venue vs small is **38 px vs 22 px** (`VENUE_R` 19), a 1.73×
  step and an **accepted deviation** from the 2× the map a11y guide asks for: at
  the home view the closest pair of separately-drawn venue pins is 39.2 px apart
  measured |dx| + |dy|, which is what two diamonds need to avoid overlapping, so
  44 px pins collide. `clusterRadius` (26 px) does not rescue a larger value —
  it only guarantees separated pins are 26 px apart. The distance is a property
  of the current venue set: re-measure if the sheet gains venues.

  Inside the featured square, the sponsor's `mark` is drawn **contain-fit and
  centred** with a 1 px paper inset inside the keyline — about 21 CSS px of
  picture. One canvas image is registered per featured sponsor, after that
  mark's `Image` loads from its precached path, and the layer's `icon-image`
  selects by sponsor id. **Nothing on the map waits for a mark**: the layer
  draws the empty square from the first frame and the marks replace it as they
  arrive, so one bad asset costs one sponsor its picture (plus a console
  warning) rather than emptying the layer.

  Legend swatches are sized so the drawn shape matches the pin it
  ...
  **The key list below the map is three sections under visible headings**, in
  ...
  highlight, recenter with `easeTo` (a jump under `prefers-reduced-motion`),
  open the sheet. The camera check the venue cards need does not apply —
  sponsors never cluster and are never displaced, so a sponsor always has a pin
  of its own — but the leader zoom is still the floor, and a card never zooms
  the map out.
  ...
  **A venue pin reserves the diamond's area, not its bounding box.** Collision
  ...
  Only venue pins have blockers. A transit or generic sponsor pin's overhang is
  ~10 px on the diagonal against a venue pin's ~14, and two more layers each was
  not worth it; if that changes, the pattern is here to copy. The featured
  square needs no blocker at all — an axis-aligned square *is* its own bounding
  ...
  **A transit stop that cannot clear a venue pin is displaced the same way,
  ...
  every other pin — venue, sponsor, and stop — across that whole range.

  Every one of those checks is the same |dx| + |dy| measure, so each shape
  contributes its **L1 radius** and two shapes clear at the sum: a diamond of
  half-diagonal R contributes R (its tip is the L1 extreme), and an
  axis-aligned square of side s contributes s (its corner, s/2 + s/2). So a
  stop clears a generic sponsor at 2·`SMALL_R` as it always did, and a featured
  sponsor at `SMALL_R` + `FEATURED_SIDE`. For a square that test is
  **sufficient but not tight** — two squares also clear at
  max(|dx|,|dy|) ≥ side, which the L1 form does not capture — so a stop can be
  pushed a few pixels further than it strictly had to be. That is the right way
  to be wrong: it costs displacement, never a mark under a transit diamond, and
  it keeps one measure across the file instead of two that have to agree. The
  ...
  **Every name layer offers the four corners.** A pin whose four sides are all
  ...
  pin it belongs to and no ownership exception is needed.

  **Sponsor names keep one clearance for both axes, and it is the featured
  square's.** The pull-in the venue pin gets is a property of a diamond — its
  edge crosses the 45° ray at half its radius — and a square has no such
  slack: its ink runs all the way to (side/2, side/2), so a diagonal offset
  measured to a diamond of the same ink lands *inside* the square's own
  collision box, where the placement pass rejects it and the name silently
  disappears. Both sponsor kinds share one name layer, so the offset is
  `FEATURED_SIDE / 2 + NAME_CLEAR_PX` for all of them — the wider of the two
  boxes, as the old single clearance was.
...
- **Keyboard map panning**: the map canvas is focusable (`tabindex="0"`,
  ...
  Every pin still has a keyboard and screen-reader path to its sheet: venues and
  pinned sponsors through the visible key list below the map, and transit stops
  through `#map-pin-alt`, a visually-hidden list holding one button per drawn
  stop (named with the stop name and its lines) that opens the same sheet a tap
  would. Those buttons un-hide while
  focused, skip-link style, so a sighted keyboard user can see where focus is.
  **`#map-pin-alt` holds transit and nothing else.** Sponsors left it when they
  gained key-list cards (2026-09-05): a sponsor with a visible button and a
  hidden one is two stops for a screen reader and one pin.
...
- `[data-testid="map-canvas"]` on the map's container element, and
  ...
  Image ids are part of it too, since a pin's drawn size is only readable from
  `map.style.getImage(id)`: `pin-venue`, `pin-transit`, `pin-cluster`,
  `pin-sponsor-generic`, and `pin-sponsor-featured` — the **empty** featured
  square, which stays registered as the fallback even once the per-sponsor
  images `pin-sponsor-featured-<sponsor id>` have replaced it on the layer. Each
  carries 2 px of canvas bleed per side, so the drawn size is
  `data.width / pixelRatio - 4`: a diamond's full diagonal, or the square's
  side.
  Source ids:
...
- `.pin-alt-btn[data-kind="transit"][data-id="<id>"]` on each button
  in the visually-hidden pin list, one per drawn transit stop and nothing else
- `.venue-key-btn[data-venue-id="<id>"]` on each venue card in the key list —
  venue cards only; a sponsor card must never answer to this selector
- `.sponsor-key-btn[data-sponsor-id="<id>"][data-featured="true"|"false"]` on
  each sponsor card, and the ids `#map-key` (the whole key), `#featured-key-list`
  and `#sponsor-key-list` on the two sponsor sections. An absent section id
  means that section is empty, which is how it renders
