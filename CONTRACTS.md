...
site/                   # the deploy root; everything in here ships
  index.html
  css/app.css
  js/*.js               # ES modules (except sw-register.js and performers-embed.js, classic)
...
## Performers embed contract (Squarespace page, not the app)

`site/js/performers-embed.js` is the one file in `site/` the app never loads. A
classic script — no imports, it runs on www.midwaymusicandart.org, outside this
repo's module graph — loaded by a one-line code block on the organizers'
performers page, where it renders that page's accordion from this site's
published `data/content.json`. README has the operator procedure. It is
precached with everything else in `site/`; accepted, not special-cased.
...
- `kind === "vendor"` events are excluded; the rest sort by `title` with
  `localeCompare` at `sensitivity: 'base'` (case and accent folded).
- Per event: `title` → the item title; `description` → the body, one paragraph
  per newline-separated line, **always text nodes, never innerHTML**; `url` → a
  "Website" link (`target="_blank"`, `rel="noopener"`) when non-empty and its
  scheme is `https:`, `http:` or `mailto:`. A missing `url` key reads as `""`,
  so the script also works against a deploy built before the column existed.
...
- Item ids are `performer-<event id>`, with `button-`/`dropdown-` prefixed
  variants for the ARIA wiring. `#performer-<id>` on load or on `hashchange`
  opens that item through the same click path and scrolls to it.
...
**Fail closed.** The fetch, the parse, an empty performer list, a missing
accordion, a missing role element: all fail before the DOM is touched, and the
swap into the page is one synchronous batch. On any failure the page keeps
exactly what the organizers authored and the script logs
...
removed, plus a content fixture — served by route interception on the fixture
server's origin, so the suite stays off the network.
